...
function Card({ tarefa, buscarTarefas, usuarios }) {
  ...
  const salvarAluguel = async () => {
    await atualizarTarefa('andamento', aluguelData);
  ...
  return (
    <div className="tarefa-card">
      ...
      {tarefa.status === 'a fazer' && (
        <>
          <button onClick={() => alterarSituacao('andamento')}>Iniciar</button>
          <button onClick={() => alterarSituacao('finalizado')}>Concluir</button>
        </>
      )}
